        ...
        <nav style={{
          ...
          {NAV_SECTIONS.map(({ label, items }) => (
            <div key={label}>
              ...
              {items.map(({ id, title }) => (
                <button key={id} onClick={() => setActiveTab(id)} style={navItem(activeTab === id)}>
                  {title}
                </button>
              ))}
            </div>
    ...
    <div>
      <div style={{ marginBottom: "0.75rem" }}>
        ...
        <h1 style={{ fontSize: "clamp(36px, 5vw, 64px)", fontWeight: 800,
          letterSpacing: "-0.02em", color: "var(--text-h)", lineHeight: 0.95, marginBottom: "0.625rem" }}>
          {str(s.brand_name)}
        </h1>
        {s.legal_name && (
          <p style={{ fontSize: 13, color: "var(--text-s)", fontFamily: "monospace", marginBottom: "1rem" }}>
            {str(s.legal_name)}{s.cin ? ` · CIN: ${str(s.cin)}` : ""}
          </p>
        )}
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: "1.75rem" }}>
        ...
        {bool(s.is_profitable) && <Chip green>✓ Profitable</Chip>}
        {s.industry    && <Chip blue>{str(s.industry)}</Chip>}
        {s.hq_city     && <Chip gray>{str(s.hq_city)}</Chip>}
        {s.glassdoor_rating && <Chip amber>Glassdoor {str(s.glassdoor_rating)}/5</Chip>}
      </div>
      ...
      <div style={{ border: "1px solid var(--border)", borderRadius: 8, padding: "1.5rem",
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--text-m)", lineHeight: 1.6 }}>
            {str(s.stage).replace(/_/g," ")} stage weights applied.
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
        {([
          ...
          <div key={k} style={{ padding: "9px 14px", background: i%2===0?"#fff":"var(--bg-soft)",
            borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 1,
        background: "var(--border)", border: "1px solid var(--border)", borderRadius: "8px 8px 0 0", overflow: "hidden" }}>
        {([["Videos", videos.length],["Own Channel", videos.some(v=>v.is_own_channel)?"Yes":"No"],
           ["Latest", videos[0]?.published_date?.slice(0,7)||"—"],["Types",[...new Set(videos.map(v=>v.video_type))].length]
      ...
      <div style={{ border:"1px solid var(--border)", borderTop:"none", borderRadius:"0 0 8px 8px", overflow:"hidden" }}>
        {videos.map((v,i)=>(
          <div key={i} style={{ display:"grid", gridTemplateColumns:"28px 1fr auto", gap:12,
            padding:"10px 1.25rem", borderBottom:"1px solid var(--border)", alignItems:"start",
            ...
            <span style={{ fontSize:9, fontFamily:"monospace", textTransform:"uppercase",
              ...
              background: typeBg[v.video_type]||"var(--bg-soft)", color: typeClr[v.video_type]||"var(--slate)",
    ...
    <div style={{ background:"#fff", border:"1px solid var(--border)", borderRadius:8, padding:"1rem" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8, marginBottom:8 }}>
        ...
        <span style={{ fontSize:9, fontFamily:"monospace", textTransform:"uppercase",
          ...
          background:"var(--bg-soft)", color:"var(--slate)", whiteSpace:"nowrap", flexShrink:0 }}>
        ...
        <div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"1rem", marginBottom:"1rem" }}>
            {([
              ...
              ["Would Recommend",   s.glassdoor_recommend ? `${str(s.glassdoor_recommend)}%` : null, "var(--green)"],
          ...
          {s.glassdoor_themes&&<p style={{ fontSize:13, color:"var(--text-m)", lineHeight:1.65 }}>{str(s.glassdoor_themes)}</p>}
        </div>
      </div>
    </div>
  )
          ...
          <thead>
            <tr style={{ background:"var(--bg-soft)", borderBottom:"1.5px solid var(--border-md)" }}>
              {["Field","Pack","Applicability","Source","Confidence"].map(h=>(
                <th key={h} style={{ textAlign:"left", fontFamily:"monospace", fontSize:10,
                  textTransform:"uppercase", letterSpacing:"0.06em", color:"var(--text-xs)",
                  fontWeight:500, padding:"7px 12px" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
...
function Chip({ children, navy, blue, green, amber, gray }: { children:React.ReactNode } & Record<string,boolean|undefined>) {
  ...
  return (
    <span style={{ display:"inline-flex", alignItems:"center", fontFamily:"monospace", fontSize:10,
      letterSpacing:"0.05em", textTransform:"uppercase", borderRadius:4, padding:"3px 8px",
...
function DQBar({ pct }: { pct?:number }) {
  if (!pct) return null
  return (
    <div style={{ display:"flex", alignItems:"center", gap:12, background:"var(--green-lt)",
      border:"1px solid var(--green-bd)", borderRadius:8, padding:"10px 14px", marginBottom:"1.25rem" }}>
      <span style={{ fontFamily:"monospace", fontSize:10, textTransform:"uppercase",
        letterSpacing:"0.08em", color:"var(--green)", whiteSpace:"nowrap", fontWeight:500 }}>Data Quality</span>
...
function SecHeader({ title, tag }: { title:string; tag?:string }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:"1.5rem" }}>
      <span style={{ fontFamily:"monospace", fontSize:10, color:"var(--blue)", background:"var(--blue-lt)",
        border:"1px solid var(--blue-md)", borderRadius:4, padding:"2px 8px", letterSpacing:"0.05em" }}>{tag||"—"}</span>
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ textAlign:"center", maxWidth:400 }}>
        ...
        <button onClick={onBack} style={{ background:"var(--navy)", color:"#fff", border:"none",
          borderRadius:6, padding:"8px 18px", fontSize:13, cursor:"pointer" }}>← Back to leaderboard</button>
      </div>
    </div>
  )
}
...
const navItem = (active:boolean): React.CSSProperties => ({
  display:"flex", alignItems:"center", gap:8, padding:"6px 1.25rem", fontSize:13,
  color:active?"var(--navy)":"var(--text-m)", borderLeft:`2px solid ${active?"var(--blue)":"transparent"}`,
  background:active?"var(--blue-lt)":"transparent", cursor:"pointer", border:"none",
  width:"100%", textAlign:"left", fontWeight:active?500:400, transition:"all 0.12s"
})
